...
if ( ! class_exists( 'WooPayKCPBase' ) ) {
	class WooPayKCPBase extends WooPayKCP {
		...
		private function woopay_init() {
			...
			add_action( 'add_woopay_kcp_virtual_bank_information', array( $this, 'add_virtual_information' ), 10, 2 );
			add_action( 'woopay_kcp_virtual_bank_schedule', array( $this, 'woopay_cancel_unpaid_virtual_bank_orders' ) );
			...
			// Classes
			include_once( 'class-woopay-kcp-actions.php' );
			include_once( 'class-woopay-kcp-payment.php' );
			include_once( 'class-woopay-kcp-refund.php' );

			// API
			include_once( 'class-woopay-kcp-api.php' );

			// Payment Methods
			include_once( 'methods/class-woopay-kcp-card.php' );
			include_once( 'methods/class-woopay-kcp-transfer.php' );
			include_once( 'methods/class-woopay-kcp-virtual.php' );
			include_once( 'methods/class-woopay-kcp-mobile.php' );
		}
		...
		public function woopay_add_email_class( $emails ) {			
			$emails[ 'WooPayKCPEmail' ] = include( 'class-woopay-kcp-email.php' );
		...
		public function woopay_cancel_unpaid_virtual_bank_orders() {
			...
			$virtual_bank_settings = get_option( 'woocommerce_kcp_virtual_settings', null );
			...
			if ( $unpaid_orders ) {
				foreach ( $unpaid_orders as $unpaid_order ) {
					...
					$expiry_date = get_post_meta( $unpaid_order, '_woopay_kcp_expirydate', true );
		...
		public function for_translation() {
			array(
				__( 'Korean Payment Gateway integrated with KCP for WooCommerce.', $this->woopay_domain )
			);
		}
	}

	return new WooPayKCPBase();
}